(function() {
	...
	uniqueField.$inject = ['$q', '$parse', '$stateParams'];
	function uniqueField($q, $parse, $stateParams) {
		var uniqueFieldDirective = {
			...
			link: function(scope, elem, attrs, ctrls) {
				...
				ngModelCtrl.$asyncValidators.uniqueField = function(value) {
					if(!value || value === '' || value === $stateParams.id) return $q.when(true);
					//funkciji prosleđujemo context (scope) u kom se izvršava i proširujemo ga parametrima i izvršavamo funkciju
					return expfn(scope, {'value':value}).$promise.then(function() {
